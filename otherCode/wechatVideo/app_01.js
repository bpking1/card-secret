/*需要添加的功能是：
1.记录当前微信号的身份，记录当前账号发送过的视频号账号，如果判定这个微信已经给这个视频号发过了，那么就跳过
2.白名单机制，针对白名单直接跳过，不发送
3.最好像游戏外挂一样，左上角有日志 //TODO ，当前账号今日发送了多少条，给谁发送了 
4.可自行设置循环次数
*/
...
for (let i = 0; i < loopCount; i++) {
    ...
    let accountTextView = className("android.widget.TextView").depth(21).clickable().find().filter(
        function (item) {
            if(item.bounds){
